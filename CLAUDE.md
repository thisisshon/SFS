...
7. **The hero is standardised - V4 dark is the ONLY surface, in THREE types under one `<Hero>`** (near-black + flipped soft-light texture + low olive glow, breadcrumb folded into `.hero-inner`, parallax landing; geometry defined once in `global.css`): **Type 1 one-column** (default) - crumb→h1→lead→cta stacked left, centred in the tall band. **Type 2 two-column** - add an `aside` slot → `.hero-grid`: the LEFT column is Type 1 verbatim, the aside sits spaced apart on the right (56px gap), collapses to a stack ≤980px. **Type 3 calculator** - pass `calc` → `.hero-calc`, every calc page (hub + details) pinned to ONE fixed height (408px = tallest calc hero). Light variant / `.hero-compact` / `.calc-hero` are all discarded. Homepage keeps its bespoke video hero. → [#rule-7](docs/conventions.md#rule-7)
