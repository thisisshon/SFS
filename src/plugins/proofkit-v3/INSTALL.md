...
the `BaseLayout` import path + the `seo` prop name to the host's layout API): `/review3` (login),
`/reviewdash3` (admin dashboard), and `/teamdash3` (per-team dashboard).

`src/pages/review3.astro`
```astro
---
...
const offSeo = { title: 'Not Found', description: 'Not available.', path: '/review3', noindex: true };
---
{PROOFKIT_ENABLED
...
const offSeo = { title: 'Not Found', description: 'Not available.', path: '/reviewdash3', noindex: true };
---
{PROOFKIT_ENABLED
...
`src/pages/teamdash3.astro` — the per-**team** dashboard (new in 2.5.0). Identical shape; one route
...
const offSeo = { title: 'Not Found', description: 'Not available.', path: '/teamdash3', noindex: true };
---
{PROOFKIT_ENABLED
...
expose the printed URL to the build as `PUBLIC_REVIEW_WORKER_URL_V3` (CI env var, or a git-ignored
...
- **`TEAM_KEYS` drive per-team access.** Set the per-team reviewer keys in `wrangler.toml` →
  ...
  reviewer and **scopes** the team-only reads on `/teamdash3` — the Worker returns that team's comments
...
- **Admin sign-in has two doors.** Direct password login at `/review3`, **or** the `/teamdash3` login
...
### 6. Verify
`npm run build` should pass. With `PROOFKIT_ENABLED = true`: open `/review3` → sign in → it arms
review mode and lands on the dashboard. Now open any content page → a **Comment** button appears →
click → (Team + Key login) → grayscale review mode → click to comment. Before signing in at `/review3`,
...
route seam — create `src/pages/teamdash3.astro` (step 3) when upgrading from ≤2.4.0, and re-deploy the
...
> project's shared layout with the correct import path; (2) create the `src/pages/review3.astro`,
> `src/pages/reviewdash3.astro`, and `src/pages/teamdash3.astro` route shims, adapting the `BaseLayout`
...
> confirm it passes, and confirm `?review3=1` shows the Comment button on a page.
