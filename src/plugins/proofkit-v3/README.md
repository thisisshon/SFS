...
- `true` — the overlay loads (dormant) on every page; `/review3` + `/reviewdash3` + `/teamdash3` work.
...
until someone signs in at `/review3`, which arms review mode for the tab (`reviewMode3`); the
Comment dock never appears before that. `?review3=0` signs out. Plus the two Worker passwords (below).
...
- Sign in at `/review3` to arm the tab; the host's back-to-top FAB (see `HIDE_SELECTORS`) is then
  replaced by a **Comment** button on every page. Before sign-in nothing renders (no Comment
  button), and it never arms on the tool's own `/review3` + `/reviewdash3` pages.
...
- Click any element to **add a comment**: name, free-text, and — for the Content team — a
  ...
  **replies** on each pin.
- A comment navigator (count + prev/next) and deep links (`#c3=<id>`) that jump to a pin.
- **Add-only** — comments are never edited or deleted from the page (that's admin-side).

**Admin dashboard** (`Login.astro` = `/review3`, `Dashboard.astro` = `/reviewdash3`)
- **Two doors in.** Either sign in directly at `/review3` (the password gate; on success it redirects
  to `/reviewdash3`, which keeps its own gate so hitting it directly still prompts), **or** pick
  **Builder** in the `/teamdash3` login dropdown and enter the admin password — that identity
  (`config.ts` `ADMIN_TEAM = 'Builder'`) maps to admin and redirects to `/reviewdash3`. Builder is a
...
**Team dashboard** (`TeamDashboard.astro` = `/teamdash3`) — *new in 2.5.0*
...
- **Reviewer key** — add comments + read a page's pins, and (on `/teamdash3`) read the signing team's
...
**Storage modes**
- **Shared/live** — when `WORKER_URL` is set (via `PUBLIC_REVIEW_WORKER_URL_V3`), everyone reads/writes
...
proofkit/
  ...
  Login.astro      The /review3 auth gate (guts; rendered by the route shim).
  Dashboard.astro  The /reviewdash3 ADMIN dashboard (guts; rendered by the route shim).
  TeamDashboard.astro  The /teamdash3 per-TEAM dashboard (guts; rendered by the route shim).
  worker/
...
| Login route | `src/pages/review3.astro` | ~15-line shim → `<ProofkitLogin />` when enabled, stub when off |
| Admin dashboard route | `src/pages/reviewdash3.astro` | ~15-line shim → `<ProofkitDashboard />` when enabled, stub when off |
| Team dashboard route | `src/pages/teamdash3.astro` | ~15-line shim → `<ProofkitTeamDashboard />` when enabled, stub when off |
| Worker URL (CI) | build env | bakes `PUBLIC_REVIEW_WORKER_URL_V3` into the build |
...
6. **Wire it up** — expose that URL to the build as `PUBLIC_REVIEW_WORKER_URL_V3` (CI env var, or a
