 ...
 * overlay stops loading on every page and the /review3 + /reviewdash3 routes render
 ...
 * localStorage demo mode. Injected at build time from PUBLIC_REVIEW_WORKER_URL_V3 —
 * the .astro adapters bridge this value to the core via `window.PROOFKIT3_WORKER_URL`
 * (see any adapter's inline script). See ./INSTALL.md.
 */
export const WORKER_URL: string = import.meta.env.PUBLIC_REVIEW_WORKER_URL_V3 || '';

/** SEO for the login route (/review3) — noindex, it's an internal tool. */
...
/** SEO for the admin dashboard route (/reviewdash3) — noindex. */
...
/**
 * SEO for the per-team dashboard route (/teamdash3) — noindex. A team signs in with
