 ...
 *   - Astro adapters inline `window.PROOFKIT3_WORKER_URL` from the env var.
 *   - Standalone html sets the same global in a <script> before core/*.js loads.
 * ------------------------------------------------------------------------ */
export const WORKER_URL =
  (typeof window !== 'undefined' && window.PROOFKIT3_WORKER_URL) || '';
 ...
 * /reviewdash3, /teamdash3 — shares this single session: the { team, key } chosen at
...
export function getSession() {
  try {
    let team = sessionStorage.getItem('pk3Team') || '';
    let key = sessionStorage.getItem('pk3Key') || '';
    if (!key) {
      ...
      const lTeam = localStorage.getItem('pk3Team') || '';
      const lKey = localStorage.getItem('pk3Key') || '';
      if (lKey) { team = lTeam; key = lKey; try { sessionStorage.setItem('pk3Team', team); sessionStorage.setItem('pk3Key', key); } catch {} }
    }
    return { team, key };
  } catch { return { team: '', key: '' }; }
}
export function setSession(team, key) {
  try { sessionStorage.setItem('pk3Team', team); sessionStorage.setItem('pk3Key', key); } catch {}
  try { localStorage.setItem('pk3Team', team); localStorage.setItem('pk3Key', key); } catch {} // shared → adoptable by new tabs
}
export function clearSession() {
  try { sessionStorage.removeItem('pk3Team'); sessionStorage.removeItem('pk3Key'); } catch {}
  try { localStorage.removeItem('pk3Team'); localStorage.removeItem('pk3Key'); } catch {}
}
 ...
 * retired. Runs ONCE per browser (guarded by `pk3DemoReset`): wipes any prior local
...
export function ensureDemoReset() {
  try {
    ...
    if (localStorage.getItem('pk3DemoReset') === '1') return; // clear once, then never again
    Object.keys(localStorage).forEach((k) => {
      if (k.indexOf('rvc3:') === 0 || k === 'rvc3-notifications' || k === 'pk3DemoSeeded') localStorage.removeItem(k);
    });
    localStorage.setItem('pk3DemoReset', '1');
  } catch {}
}
...
const THEME_KEY = 'pk3Theme';              // localStorage cache — instant, no-flash first paint
