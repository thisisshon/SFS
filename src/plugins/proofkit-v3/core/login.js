  ...
  (() => {
    ...
    async function tryLogin(input) {
      ...
      try {
        ...
        sessionStorage.setItem('reviewMode3', '1'); // arm the on-page Comment dock site-wide
    ...
    async function init() {
      ...
      if (existing) {
        try { await validate(existing); sessionStorage.setItem('reviewMode3', '1'); location.replace(DASH); return; }
