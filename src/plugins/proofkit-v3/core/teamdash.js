  ...
  (() => {
    ...
    // Admin override: Builder (admin) can open ANY team's board via /teamdash3?team=<T>
    ...
    function localResubmit(rec) {
      ...
      let ex = []; try { ex = JSON.parse(localStorage.getItem('rvc3-notifications') || '[]'); } catch {}
      ex.push(notif);
      localStorage.setItem('rvc3-notifications', JSON.stringify(ex));
    ...
    function localNotifs(t) {
      let arr = [];
      try { arr = JSON.parse(localStorage.getItem('rvc3-notifications') || '[]'); } catch {}
    ...
    function localMarkRead(ids, read = true) {
      ...
      if (updated) localStorage.setItem('rvc3-notifications', JSON.stringify(arr));
          ...
          `<div class="tmd-card-row">` +
            `<div class="tmd-card-main">` +
              ...
              `<div class="tmd-card-actions">` +
                `<a class="tmd-openpin" href="${esc(root.page.path)}?review3=1#c3=${id}" target="_blank" rel="noopener">Open Pin</a>` +
      ...
      host.innerHTML =
        ...
        `<article class="tmd-detail">` +
          ...
          `<div class="tmd-detail-chips">${statusChip(c)}${c.toTeam ? '<span class="tmd-from">with ' + teamChip(c.toTeam) + '</span>' : ''}` +
            `<a class="tmd-slug" href="${esc(c.page.path)}?review3=1#c3=${esc(c.id)}" target="_blank" rel="noopener">Open pin</a></div>` +
          (reopened
      ...
      return `<article class="tmd-note${unread ? ' is-unread' : ''}">` +
        ...
        `<div class="tmd-note-body">` +
          ...
          `<div class="tmd-note-meta">` +
            ...
            (n.commentId ? `<a class="tmd-openpin" href="${esc(n.path || '/')}?review3=1#c3=${esc(n.commentId)}" target="_blank" rel="noopener">Open Pin</a>` : '') +
          `</div>` +
        `</div>` +
    ...
    async function tryLogin() {
      ...
      if (t === ADMIN_TEAM && !OVERRIDE) { location.replace('/reviewdash3'); return; }
    ...
    function init() {
      ...
      if (s.key && s.team === ADMIN_TEAM && !OVERRIDE) { location.replace('/reviewdash3'); return; }
    ...
    function mountAdminBar() {
      ...
      bar.innerHTML = `<span class="tmd-adminbar-txt">Admin view — <b>${esc(OVERRIDE)}</b> team board (full access)</span>` +
        `<a class="tmd-adminbar-back" href="/reviewdash3">← Back to admin</a>`;
      app.prepend(bar);
    ...
    if (upgrade) upgrade.addEventListener('click', (e) => {
      e.preventDefault();
      clearSession();
      location.href = '/reviewdash3?login=builder';
    });
