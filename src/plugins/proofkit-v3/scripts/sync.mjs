...
//
// Copies the self-contained Proofkit package (src/plugins/proofkit-v3/) between
...
const PKG_ROOT = dirname(scriptDir);                        // <pkg> = src/plugins/proofkit-v3
const PKG_REL = join('src', 'plugins', 'proofkit-v3');     // package location inside any project
...
Package copied. The target still needs these host seams (see ${PKG_REL}/INSTALL.md):
  ...
  2. Route shim:  ${join('src', 'pages', 'review3.astro')}
  3. Route shim:  ${join('src', 'pages', 'reviewdash3.astro')}
Then set PUBLIC_REVIEW_WORKER_URL_V3 and deploy the worker.
