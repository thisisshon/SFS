...
1. Sign in to the admin dashboard (`/reviewdash`).
...
2. **Route stubs** — `src/pages/`: `review.astro`, `review-guide.astro`,
   `reviewdash.astro`, `teamdash.astro`, and the `reviewdash/` folder
...
4. **Sitemap filter** — `astro.config.mjs`: drop the
   ...
   `filter` regex.
5. **Build/deploy** — remove `PUBLIC_REVIEW_WORKER_URL` from
